...
2. confirm `AGENTGUARD_REPO_TOKEN` still has read access to the private AgentGuard repository
3. rerun the workflow without changing thresholds or scenarios
...
3. do not enable embedded fallback for production sign-off; fix the OPA path and rerun validation
...
3. do not approve enforcement changes until the artifact set is complete
...
3. only restore the newer release after a clean production-validation run