...
| STANAG 5527 / NFFI       | NATO Friendly Force Information | ✅ XML serializer module implemented                  | Receiving-system conformance evidence still required          |
...
| Cursor on Target (CoT)   | CoT event schema (MITRE/TAK)    | ✅ XML serializer module implemented                  | Receiving-system conformance evidence still required          |
...
| CoT XML export                                   | ✅ Implemented                  | `GET /v1/interop/cot/vessel/{mmsi}`, `GET /v1/interop/cot/alert/{alert_id}` | CoT XML                             | Operational receiving-system validation still pending   |
| STANAG 5527 / NFFI XML                           | ✅ Implemented                  | `GET /v1/interop/nffi/vessel/{mmsi}`, `GET /v1/interop/nffi/alert/{alert_id}` | STANAG 5527 XML                     | Operational receiving-system validation still pending   |
...
| --------------------------- | ------------------------------------------ | -------------------------------------------------------- | -------------------------------- |
| CoT/STANAG receiving-system conformance evidence incomplete | Medium — procurement evidence gap | Run staged integration tests against TAK/NATO receiver stack and archive outputs | In progress (D-04) |
...
| Artefact                      | Location                                                               |
| ----------------------------- | ---------------------------------------------------------------------- |
| API contracts                 | `docs/product/API_DOCUMENTATION.md`                                    |
