...
> Historical note
> This document records a prior fix cycle and still references the older `backend` / `frontend` path layout. Treat it as implementation history, not the authoritative current operating guide.
