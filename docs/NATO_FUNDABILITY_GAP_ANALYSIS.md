...
| Security evidence pack                     | ⚠️ Partial (no IR runbook, no pen test) | **BLOCKER** for NCIA tender                                |
| Interop profile                            | ⚠️ JSON only, no NATO XML               | **BLOCKER** for STANAG compliance                          |
...
| ML/AI scoring                              | 🔴 Stub (placeholder comment)           | **BLOCKER** — DIANA funds deep tech, not rule engines      |
| Live AIS stream                            | 🔴 Stub (logs "started", does nothing)  | **BLOCKER** — can't demo to evaluators                     |
...
- No ML capability at all (scoring.py is literally `# Placeholder for ML`)
- No named operational user or letter of intent
- No live data means no TRL 5 evidence
...
- Security evidence incomplete (no pen test, no IR runbook, no MFA)
- No STANAG serializers implemented
- No classified environment deployment story
...
**Current state:** Not implemented. Not in backlog.
...
**Effort:** 4-6 weeks for watchlist matching + STS detection. Flag hopping requires historical MMSI correlation database.
...
**Current state:** No pen test (planned 2026-06-01), no incident response runbook (IR-01 missing), no MFA, no container image scanning.
...
- **Incident response runbook (IR-01)** — Document, test, and rehearse
- **MFA implementation** — TOTP (RFC 6238) on auth module (Sprint 8 is too late — bring forward)
...
**Effort:** 2-3 weeks for IR runbook + MFA + container scanning; pen test is external dependency.
...
- [ ] Write IR runbook (GAP-10a), implement TOTP MFA (GAP-10b), add Trivy to CI (GAP-10c)
...
- [ ] Implement sanctions watchlist matching (GAP-09a)
...
| Security evidence complete                      | ⚠️ Missing IR runbook, pen test, MFA                          | ✅ Full evidence pack with pen test report                        |
