...
| Sharing route auth         | A.5.15, A.9.4       | PR.AC-3            | CC6.3           | Authenticated collaboration routes         | `app/modules/sharing/router.py`, `tests/test_sharing_api.py`              | Backend              | Per PR         |
| COP feed access control    | A.5.15, A.9.4       | PR.AC-3            | CC6.3           | Viewer auth on shared operational feed     | `app/modules/sharing/router.py`, `tests/test_sharing_api.py`              | Backend              | Per PR         |
...
| BFF JWT verification       | A.5.17, A.8.2       | PR.AA-5            | CC6.1           | Issuer/audience/JWKS verification path     | `apps/bff/src/middleware/auth.ts`, `apps/bff/tests/auth-policy.test.ts`   | BFF                  | Per PR         |
| Classification enforcement | A.5.15, A.8.2       | PR.AC-4            | CC6.3           | Clearance gate middleware                  | `apps/bff/src/middleware/policy.ts`, `apps/bff/tests/auth-policy.test.ts` | BFF                  | Per PR         |
| Releasability enforcement  | A.5.15, A.8.2       | PR.AC-4            | CC6.3           | NATO-tag gate middleware                   | `apps/bff/src/middleware/policy.ts`, `apps/bff/tests/auth-policy.test.ts` | BFF                  | Per PR         |
| Auth context normalization | A.5.15, A.8.2       | PR.AA-4            | CC6.3           | Viewer claim normalization endpoint        | `apps/bff/src/routes/auth.ts`, `apps/bff/tests/auth-policy.test.ts`       | BFF                  | Per PR         |
...
| Export boundary control    | A.12.1              | PR.IP-1            | CC8.1           | Explicit export limit on alerts            | `app/api/v1/alerts.py`, `tests/test_interoperability.py`                  | Platform             | Per PR         |
| WebSocket tenant isolation | A.5.15, A.9.4       | PR.AC-3            | CC6.3           | Org-aware alert-status broadcast routing   | `app/infrastructure/ws/manager.py`, `tests/test_websocket_auth.py`        | Backend              | Per PR         |
...
| ------------------------- | ----------------------------------------------------------------------- | ------------ |
| Cross-tenant data leakage | Tenant isolation (org_scope), sharing auth, WS org routing              | Low          |
...
| Privilege escalation      | KNOWN_FEATURES allowlist, org scope, classification/releasability gates | Low          |
...
# Focused collaboration / tenant boundary validation
...
# BFF TypeScript type check (confirms auth/policy gates compile)
...
- Collaboration sharing remains payload-backed rather than persisted entity-backed, so provenance is improved but not yet first-class.
