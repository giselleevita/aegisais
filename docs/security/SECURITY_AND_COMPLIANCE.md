...
- API RBAC is enforced on authenticated roles and org-scoped query paths.
- BFF JWT middleware normalizes role, clearance, releasability, and license claims for downstream policy checks.
- Sensitive BFF routes can enforce minimum classification and releasability tags through dedicated middleware.
- Collaboration routes now require authenticated user context and derive sharing organisation from the caller.
- WebSocket alert-status events now route by tenant for org-scoped payloads.
...
- Existing audit plumbing is present; alert export coverage is implemented, while some privileged layer-access denial logging still needs explicit endpoint-level completion.
...
| AC-07      | BFF JWT verification and claim normalization | A.9.2         | PR.AC-4  | CC6.3    | ✅     | `apps/bff/src/middleware/auth.ts`, `apps/bff/src/routes/auth.ts`, `apps/bff/tests/auth-policy.test.ts` | BFF       |
| AC-08      | Classification and releasability gates       | A.9.2         | PR.AC-4  | CC6.3    | ✅     | `apps/bff/src/middleware/policy.ts`, `apps/bff/tests/auth-policy.test.ts`                              | BFF       |
| AC-09      | Collaboration route authentication           | A.9.2         | PR.AC-3  | CC6.3    | ✅     | `app/modules/sharing/router.py`, `tests/test_sharing_api.py`                                           | Backend   |
| AC-10      | Tenant-aware WebSocket routing               | A.9.4         | PR.AC-3  | CC6.3    | ✅     | `app/infrastructure/ws/manager.py`, `tests/test_websocket_auth.py`                                     | Backend   |
...
| DP-04      | Bulk export boundary control                 | A.12.1        | PR.IP-1  | CC8.1    | ✅     | `app/api/v1/alerts.py`, `tests/test_interoperability.py`                                               | Platform  |
...
| --------------------------------------- | -------------------------- | ------------------------------------------------------------------ | -------------- |
| Cross-tenant data leakage               | Authenticated API call     | AC-02 (org_scope), AC-09 (sharing auth), AC-10 (WS tenant routing) | Low            |
...
| Privilege escalation via forged license | Bearer token               | AC-06 (KNOWN_FEATURES allowlist), AC-07, AC-08                     | Low            |
...
- Some privileged layer-access and policy-denial audit events still need endpoint-level completion.
- MFA (AC-03) planned Sprint 8.
- Container image scanning (SC-05) planned next DevSecOps sprint.
- Collaboration sharing still returns classified payload wrappers rather than persisted collaboration records.
